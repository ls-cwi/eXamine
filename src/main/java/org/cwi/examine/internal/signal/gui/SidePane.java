...
public class SidePane extends Snippet {
    ...
    public void draw() {
        ...
        // Draw pane backdrop.
        StaticGraphics.color(Parameters.containmentColor);
        StaticGraphics.fillRect(0, 0, paneWidth, StaticGraphics.applicationHeight());
        
        // Otherwise, draw full pane.
        if(!categoryTrail.isEmpty()) {
            // Draw category trail.
            for(Category c: categoryTrail) {
                StaticGraphics.snippet(c);
            }
    ...
    final void updateLayout() {
        double space = Parameters.spacing;
        ...
        // Otherwise, full pane.
            ...
            while(rIt.hasNext()) {
                ...
                if(repPos.y + r.dimensions().y > StaticGraphics.sketchHeight()) {
                ...
                repWidth = Math.max(repWidth, r.dimensions().x);
            }
        }
        ...
        if(!categoryTrail.isEmpty()) {
            ...
            for(Representation r: representations()) {
                paneWidth = Math.max(paneWidth, r.topLeft().x + r.dimensions().x);
            }
            paneWidth += space;
        }
    }
    ...
    public class Category extends Snippet implements Comparable<Category> {
        ...
        public void draw() {
            ...
            double arrowSize = StaticGraphics.textHeight() / 4f;
            double middle = StaticGraphics.textMiddle();
                        
            StaticGraphics.picking();
            
            StaticGraphics.translate(position);
            
            StaticGraphics.color(isHovered() ? Parameters.textContainedHoverColor :
                               active ? Parameters.textContainedHighlightColor :
                                        Parameters.textContainedColor);
            
            // State arrow.
            StaticGraphics.pushTransform();
            StaticGraphics.translate(arrowSize, middle);
            StaticGraphics.rotate(active ? (double) PI / 2f : 0);
            ...
            triangle.closePath();
            StaticGraphics.fill(triangle);
            
            StaticGraphics.popTransform();
            
            StaticGraphics.translate(StaticGraphics.textHeight() / 2f + Parameters.spacing, 0);
            
            // Label.
            StaticGraphics.text(name);
        }
        ...
        public double width() {
            ...
            return StaticGraphics.textWidth(name) + StaticGraphics.textHeight() / 2f + Parameters.spacing;
        }
