...
public class ChoiceRepresentation<E> extends Representation<Variable<E>> {
    ...
    public void draw() {
        ...
        color(contained ? Parameters.textContainedColor : Parameters.textColor);
        text(label);
        
        // Next line and ident.
        translate(Parameters.spacing + dotSize() + Parameters.spacing, textHeight() + Parameters.spacing);
        
        // Place selection dot.
        int dotIndex = values.indexOf(element.get());
        if(dotIndex >= 0) {
            color(contained ? Parameters.textContainedHighlightColor : Parameters.textHighlightColor);
            drawEllipse(-(Parameters.spacing + dotSize()) / 2f,
    ...
    public PVector dimensions() {
        double space = 0.5f * Parameters.spacing;
    ...
    private class ValueSnippet extends Snippet {
        ...
        public void draw() {
            ...
            Color color = contained ? (isHovered() ? Parameters.textContainedHoverColor :
                                          selected ? Parameters.textContainedHighlightColor :
                                                     Parameters.textContainedColor) :
                                      (isHovered() ? Parameters.textHoverColor :
                                          selected ? Parameters.textHighlightColor :
